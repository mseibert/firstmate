...
A recorded endpoint that a session death removed is recreated by the relaunch itself, in that same recorded worktree, so a missing window is never a reason to hand-create one or to start a fresh generic spawn.
