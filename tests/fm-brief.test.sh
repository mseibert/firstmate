...
# CI job that parsed every shell file under macOS's stock Bash 3.2.57 was the
# cross-version enforcement; it was removed on 2026-09-17, so
# test_no_heredoc_in_command_substitution is now the only guard for this class.
set -u
...
# CI and locally, where the issue #958/#1069 parser bug does not fire, so this is
# a weak guard on its own; test_no_heredoc_in_command_substitution carries the
# real enforcement now that the macos-stock-bash job is gone (2026-09-17).
test_script_parses() {
  local out rc
