...
1. First action: create your branch: `git checkout -b fm/dod-lens-prefix`

# Rules
1. Never push to the default branch (push only your `fm/dod-lens-prefix` branch). Never merge a PR.
2. Stay inside this worktree; modify nothing outside it.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-dod-lens.5v1Zmh/prefix-home/state/dod-lens-prefix.status'`
...
Firstmate steers you through durable message files in '/tmp/fm-dod-lens.5v1Zmh/prefix-home/state/dod-lens-prefix.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-dod-lens.5v1Zmh/prefix-home/state/dod-lens-prefix.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-dod-lens.5v1Zmh/prefix-home/state/dod-lens-prefix.inbox'/NNN.msg '/tmp/fm-dod-lens.5v1Zmh/prefix-home/state/dod-lens-prefix.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/tmp/fm-dod-lens.5v1Zmh/prefix/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md`, follow `/tmp/fm-dod-lens.5v1Zmh/prefix/bin/fm-ensure-agents-md.sh`'s self-governance contract in the same pass.
