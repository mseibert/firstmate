...
1. First action: create your branch: `git checkout -b fm/mc-local-only`
...
# Rules
1. Never push to any remote and never open a PR. Work only on your `fm/mc-local-only` branch; firstmate handles the merge into local `main`.
2. Stay inside this worktree; modify nothing outside it.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-brief-evidence/home/state/mc-local-only.status'`
...
Firstmate steers you through durable message files in '/tmp/fm-brief-evidence/home/state/mc-local-only.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-brief-evidence/home/state/mc-local-only.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-brief-evidence/home/state/mc-local-only.inbox'/NNN.msg '/tmp/fm-brief-evidence/home/state/mc-local-only.inbox'/handled/`.
...
The task is complete only when committed on your branch `fm/mc-local-only`. Do NOT push, do NOT open a PR, do NOT merge.
Keep your branch a clean fast-forward onto the current default branch - if `main` has advanced, rebase onto it so the eventual merge stays a fast-forward.
When it is implemented and committed, append `done: ready in branch fm/mc-local-only` to the status file and stop.
