...
#
# The fleet runs the self-update pass every six hours, instead of a hand-run
# /updatefirstmate. The pass
...
# is that step's tool. It never touches any other home, and a separate firstmate
# home on another machine arms the same tracked code in its own home with its
# own timer. docs/configuration.md "Self-update timer" owns the operator-facing
# contract.
#
# Usage:
...
# wrapper supplies the invoking home's substitutions.
install_unit() {  # <template> <destination>
  local template=$1 dest=$2 home
  home=$(resolve_home) || { error "cannot resolve FM_HOME $FM_HOME"; return 1; }
  fm_unit_install "$UNIT_DIR" "$template" "$dest" \
    "@FM_HOME@=$home" "@FM_SELF_UPDATE_RUN@=$RUN_BIN" \
    || { error "${FM_UNIT_ERROR:-could not install $dest}"; return 1; }
}
...
action_arm() {
  ...
  install_unit "$SERVICE_TEMPLATE" "$SERVICE_FILE" || return 1
  install_unit "$TIMER_TEMPLATE" "$TIMER_FILE" || return 1
  ...
  if [ "$FM_UNIT_CHANGED" = no ] && [ "$timer_state" = active ] && [ "$enabled_state" = enabled ]; then
    printf 'self-update timer: already armed %s (active, enabled); log %s\n' "$TIMER_UNIT" "$LOG"
    return 0
  fi

  if [ "$FM_UNIT_CHANGED" = yes ]; then
