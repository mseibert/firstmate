...
Those per-script maxima total 4929468 ms of conservative balance weight.
...
A script with no hint gets the conservative `PORTABLE_SERIAL_DEFAULT_WEIGHT_MS` default; the current 162-script lane has three such scripts, bringing its assignment weight to 5010468 ms.
...
| Lane | Script count | Estimated duration |
|---|---:|---:|
| `portable-serial-1of5` | 31 | 1002114 ms (~16.70 min) |
| `portable-serial-2of5` | 33 | 1002086 ms (~16.70 min) |
| `portable-serial-3of5` | 32 | 1002088 ms (~16.70 min) |
| `portable-serial-4of5` | 33 | 1002074 ms (~16.70 min) |
| `portable-serial-5of5` | 33 | 1002106 ms (~16.70 min) |
| imbalance | | 28 ms |

The current table is generated from the runner's retained maxima plus its default for the three unhinted scripts.
Every shard's assignment weight is its conservative worst case, 16.70 min, which is 37% of the 45-minute job cap.
...
| portable serial 1-5 | job `timeout-minutes: 45` | Each balanced shard carries 16.70 minutes of conservative assignment weight, and the worst shard measured so far ran 29m44s of test time in [run 34911370911](https://github.com/mseibert/firstmate/actions/runs/34911370911) on 2026-09-15 - `tests/fm-watch-triage.test.sh` alone took 22m17s against its 6m36s hint - in a job the 30-minute cap cancelled before its timing artifact finished uploading even though every test had passed. 45 minutes keeps the bound a hang tripwire with roughly 1.5x margin over the worst measured shard instead of the routine end of the lane. |
