...
The watcher maps the pane back to the task and skips secondmate endpoints, declared `paused:` waits, verified `captain-held` transfers, and worker-declared `done-pending-verify` waits, because a declared wait already names the human the fast escalation would report and is left to the watcher's own bounded pause cadence.
