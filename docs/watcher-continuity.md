...
The Pi branch settlement is backgrounded: after a wake is accepted by the supervision branch, the watcher starts the next pending wake's successor immediately instead of awaiting that settlement (a full model turn), so a second quick wake in a burst never finds the fleet without a live successor or a bounded restoration in progress.
The branch still settles or rejects on its own; the watcher's background monitor bounds that settlement and falls back to the main delivery path when the branch rejects, or when a bounded window elapses while the branch no longer holds its per-wake row grant ([`pi-supervision-branch.md`](pi-supervision-branch.md#components-and-their-owners) owns the branch side of that handshake).
