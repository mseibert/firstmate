...
The one in-flight-looking declaration that never escalates as a dead worker is `done-pending-verify`: that worker has already delivered its work and parks without an agent by design (the expected parked state), so its missing process is exactly as legitimate as a `done:` worker's, and the parked stale absorb on the bounded cadence holds.
...
A parked or done-pending-verify crew is decided from the TASK state, never from the pane command: the worker-declared `done-pending-verify` status verb and an authoritative `parked` current state (a run parked at a gate) both name an expected idle pane, so the stale path absorbs them on the bounded cadence instead of the wedge schedule, while a genuinely hung crew still reports unknown and surfaces immediately.
