...
Once a due PR has been green and mergeable past `config/pr-green-return`, it queues one main-owned `check` wake carrying the mandate to merge through `bin/fm-pr-merge.sh` bound to the head the scan verified; a PR held by a hard stop, an unreadable policy, or the allowlist default ask queues the hold report naming that reason instead.
