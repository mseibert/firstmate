...
Once a due PR has been green and mergeable past `config/pr-green-return`, it queues one main-owned `check` wake carrying the mandate to merge through `bin/fm-pr-merge.sh` bound to the head the scan verified, for the forges that can bind it (Forgejo `head_commit_id`, GitLab `--sha`); a PR held by a hard stop, an unreadable policy, the allowlist default ask, an unbound GitHub merge path, or a Forgejo head that stays green only through the preview waiver queues the hold report naming that reason instead.
