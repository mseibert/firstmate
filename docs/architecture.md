...
The direct-PR definition of done also makes the five-lens gate a required part of the PR body: the worker runs the five lenses over the branch diff, records each lens's result, and writes `Result: clean` only when every lens ran and no finding remains open.
