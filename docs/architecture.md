...
A bounded green-return scan (`bin/fm-pr-green-return.sh`, invoked once per poll loop beside inactive-outcome reconciliation) closes the other half of the merge path: for every task that records a `pr=` line it reads the live pull request through that task's forge, applies the merge-policy hard stops and the posture's repo table from `~/.claude/pr-policy.md` plus a built-in `.forgejo/workflows/**` sensitive ground, and keeps a durable per-task wait record so a restart never resets the wait.
Once a due PR has been green and mergeable past `config/pr-green-return`, it queues one main-owned `check` wake carrying the mandate to merge through `bin/fm-pr-merge.sh` with `--expected-head` naming the head the scan verified, for the forges that can bind it (Forgejo `head_commit_id`, GitLab `--sha`), and the merge path refuses a live head that differs from the expected one or a fresh verdict that is no longer due at that head, naming the holding hard stop; a PR held by a hard stop, an unreadable policy, the policy's repo-table hold, an unbound GitHub merge path, or a Forgejo head that stays green only through the preview waiver queues the hold report naming that reason instead.
The scan never merges: `bin/fm-pr-merge.sh` keeps its live verification and head binding, merge ownership stays main-only (`bin/fm-lease-lib.sh`), and the check kind is never offered to the Pi supervision branch.
`bin/fm-watch.sh` owns the poll-loop invocation, the scan's own cadence and budget, and the `check: pr-green-return` surfacing; the script's header owns the config schema, the evidence-backed wait start, and the hold semantics; `tests/fm-pr-green-return.test.sh` pins the detection, the wait, every hard stop, and the watcher integration.
...
<<<<<<< HEAD
A crew that declares `paused:` for a known external wait, carries a verified `captain-held` transfer, or writes the worker-declared `done-pending-verify` verb (done, awaiting firstmate/captain verification) is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
...
The declared-wait absorption has one hard exception, the captain's dead-worker reality rule: a task without a running process is not a declared wait. When the machine is idle (1-minute load below 1 and no other in-flight worker) and a recorded in-flight task's endpoint holds no live agent, the watcher escalates a `dead worker: <task>` stale wake regardless of the status declaration, including a `paused:` line - the shape of a worker OOM-killed together with its window, which otherwise leaves no pane for the stale backbone to classify. `bin/fm-watch.sh` owns the deterministic, idempotent, fail-closed rule, its spawn-freshness grace, and its kernel OOM evidence.
The in-flight-looking declarations that never escalate as a dead worker are `done-pending-verify` and a task released through `bin/fm-park.sh` ([park-release.md](park-release.md)): the first has already delivered its work and parks without an agent by design, and the second was stopped by the control plane only after its handoff was recorded, so in both cases the missing process is exactly as legitimate as a `done:` worker's and the parked stale absorb on the bounded cadence holds.
...
A secondmate's endpoint liveness is still never read at all; a mate is admitted to that same cadence only to serve a declared wait's bounded re-surface, so a forgotten `paused:`, `captain-held`, or `done-pending-verify` declaration on a mate cannot rot invisibly.
...
<<<<<<< HEAD
A declared external wait, verified captain-held transfer, or done-pending-verify wait trades that silence for one bounded recheck per pause window, naming which human the wait is on, so neither a forgotten pause, a forgotten hold, nor a forgotten verification wait can remain invisible indefinitely.
=======
...
In that status-log fallback, a declared external wait reports the distinct `paused` state with its reason, while a worker-declared `done-pending-verify` log reports the distinct `parked` state - a delivered-but-unconfirmed task is parked, exactly like a crew parked at a gate.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-wait vocabulary (a `paused:` external wait, a verified `captain-held` transfer, and a worker-declared `done-pending-verify` alike, through one combined predicate), and status-scan primitives.
=======
...
A parked or done-pending-verify crew is decided from the TASK state, never from the pane command: the worker-declared `done-pending-verify` status verb and an authoritative `parked` current state (a run parked at a gate, or a task released through `bin/fm-park.sh`) both name an expected idle pane, so the stale path absorbs them on the bounded cadence instead of the wedge schedule, while a genuinely hung crew still reports unknown and surfaces immediately.
A bare shell prompt alone proves nothing (the same lesson as fm-guard-deadworker), which is why the decision is keyed on the task state rather than on `pane_current_command`.
The away-mode daemon keeps its status-line-only classification: a run-step-parked crew whose status log never declares the wait still ages toward a possible wedge there, while the normal-mode watcher reads the authoritative parked state directly.
...
The primary checkout is healthy on its default branch or on the durable `seibert/main` fork line, and linked worktrees or secondmate homes are healthy at detached HEAD.
Only a named non-default branch other than the fork line checked out in `FM_ROOT` is a worktree tangle.

`fm-tangle-lib.sh` resolves the default branch from `origin/HEAD`, then local `main` or `master`, and classifies that named non-default primary branch as the tangle, except the durable `seibert/main` fork line that `fm-ff-lib.sh` already allows the primary to sit on.
...
The direct-PR and no-mistakes definitions of done anchor the verdict-head ordering: the last code step before a verdict request is a rebase onto the current main, the branch stays frozen until the verdict covers the head, and every verdict request is preceded by a target-base check (the intended integration branch, never the default) and a mergeable check.
The direct-PR and no-mistakes definitions of done both make the five-lens gate mandatory: direct-PR records each lens's result in the PR body it opens, while no-mistakes posts the same per-lens result as one designated PR comment after the pipeline opens the PR, and either mode writes `Result: clean` only when every lens ran and no finding remains open.
...
It also reuses an existing case variant of `CLAUDE.md`, such as `Claude.md`, as the pointer path instead of adding a second spelling beside it, and refuses two distinct variants so the pointer is not split across spellings.
...
The home's own checkout (`FM_ROOT`) on the durable `seibert/main` fork line is the one exception: a clean one is an allowed primary state, so the refresh reports it as a benign skip and never moves it, leaving that line's advancement to the self-update path.
...
<<<<<<< HEAD
The update is fast-forward only except for the durable `seibert/main` fork line's upstream reconciliation, which can merge the clean `main` mirror: dirty, diverged, offline, and wrong-branch targets are reported and left untouched, and the line's own commits are never rebased or force-moved.
=======
