...
<<<<<<< HEAD
  ...
  The watcher never blocks the restoration loop on a branch settlement: it claims an accepted wake optimistically and runs the settlement off-loop with a bounded main-delivery fallback, so a burst's second wake still gets its successor while the first turn is pending ([`watcher-continuity.md`](watcher-continuity.md#actionable-wake-ordering) owns that ordering).
=======
...
- Consistency: `bin/fm-lease-lib.sh` owns the per-task lease contract, the main-only role partition, and the deliberate CONFUSED-AGENT-GRADE threat model these guards target (captain-decided; adversarial-grade separation is out of scope and tracked as follow-up design work); `bin/fm-lease.sh` is the command surface.
  The guards are wired into `fm-send.sh`, `fm-control.sh`, `fm-teardown.sh`, and `fm-park.sh` (overlap, lease-checked, with claim serialization retained through the mutation) and `fm-pr-merge.sh`, `fm-merge-local.sh`, and `fm-spawn.sh` (main-owned, branch refused; a relaunch through `fm-control` stays branch-legal recovery).
