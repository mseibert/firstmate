...
| `fm-captain-context-watch-arm.sh` | Install, inspect, and remove the captain context watch under a systemd `--user` unit with a registered liveness check |
...
| `fm-backlog-readcheck.sh` | Read-only read-time reconciliation: stale In-flight rows, shared worktree slots, and stale `done-pending-verify` notes with `--api` |
...
| [`fm-dod-lib.sh`](../bin/fm-dod-lib.sh) | Own ship/scout worker role scope, ship definitions of done (including the shared verdict-head ordering on direct-PR and no-mistakes), and the no-mistakes `--intent` contract |
...
| `fm-ensure-agents-md.sh` | Ensure a project's real `AGENTS.md`, its `CLAUDE.md` (or an existing case variant) `@AGENTS.md` pointer, and self-governance guidance (explicit project mark documented in the helper's header and help) |
...
| `fm-verdict-wait.sh`     | Read a GitHub or Forgejo PR's review verdict the way that forge publishes it and wait, bounded, for a fresh verdict and green CI |
...
| `fm-unit-install-lib.sh` | Render and install a tracked systemd --user unit template safely for the arm helpers, refusing symlinks and rewriting only changed bytes |
| `fm-pr-poll.sh`          | Provide the byte-static watcher program for validated PR/MR-poll sidecars           |
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static merge poll |
| `fm-pr-green-return.sh`  | Surface a green own-task PR past its return wait as a bound-merge mandate or a named policy hold |
...
| `fm-cognee-context.sh`   | Query the Cognee memory store for context relevant to upcoming work and render it for the task brief ([cognee-memory-bridge.md](cognee-memory-bridge.md)) |
| `fm-ci-unprivileged.sh`   | Run a CI command as an unprivileged user when the runner executes the job as root, so permission-injected failure proofs are not vacuous |
