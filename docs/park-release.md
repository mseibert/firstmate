...
`tests/fm-park.test.sh` pins eligibility (done plus PR parks, working refuses, secondmate refuses, no pointer refuses), the run-step gate rules (an ask-user gate parks as a decision, a fix-review gate and a keyless ask-user gate refuse), idempotency, the releasing retry's restored status line, the stale-incarnation marker, the raw-harness resume, the marker contents, the status line, resume notes and marker removal, clear, list/status, the backlog note through the gate library, the manual fallback, the sweep, and the snapshot's active-versus-parked occupancy projection.
