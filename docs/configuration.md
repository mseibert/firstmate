...
Completion preserves a pull or merge request from any forge: a GitHub `/pull/<n>` link is recorded through tasks-axi's typed `--pr` flag, while any other forge's link is written into the task body as the finished work's deliverable and the close runs without that flag, for a fresh close and a replayed pending-close record alike.
...
<<<<<<< HEAD
`bin/fm-park.sh` records its release note under these same gate rules: an automatic home with compatible tasks-axi gets the note through `tasks-axi update --body-file --archive-body`, a manual home is told the exact note owed, and an automatic home whose backend cannot be read refuses the release before the worker is stopped ([park-release.md](park-release.md)).
...
The scan applies the merge policy in `~/.claude/pr-policy.md` (or `FM_PR_GREEN_RETURN_POLICY`): a repo the policy's posture holds (not in the `autonomous` rows of an `allowlist` policy, or on the `ask`/`deny` wait list of a `denylist` policy), a policy hard stop, an unreadable, unrecognized-posture, or posture-contradicting policy, or a changed-file list that cannot be proven complete holds the PR instead of merging it, and the queued wake names that reason for the captain.
An otherwise due GitHub PR is also held, because the GitHub merge path cannot bind the merge to the verified head; Forgejo (`head_commit_id`) and GitLab (`--sha`) carry the bound-merge mandate, and the mandate names the verified head with `--expected-head`, which the merge path refuses to replace with a moved one. Before merging, that path re-reads the task's live return-path verdict and refuses a mandate whose head is no longer due, so a policy hold that arrives at the same head cannot be landed by an already-queued mandate.
The record under `state/pr-green-return/<id>` holds the observed PR identity, class, reason, wait start, and notification, so a restart never resets the wait and one (head, class, reason) is reported once.
...
Bootstrap also reports a `TANGLE:` line when `FM_ROOT` is on a named non-default branch other than the durable `seibert/main` fork line; follow the printed checkout remediation rather than treating it as an installable tool problem.
...
<<<<<<< HEAD
FM_PARK_CONTROL_BIN=bin/fm-control.sh   # test override for the control plane bin/fm-park.sh stops and resumes through
FM_PARK_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader fm-park.sh eligibility uses
FM_PARK_NOW_EPOCH=      # test override for the park epoch recorded in state/<id>.parked
FM_PARK_SWEEP_LIMIT=2   # maximum park attempts in one bin/fm-park.sh sweep
FM_PARK_SWEEP_BUDGET_SECS=20   # wall-clock budget for one bin/fm-park.sh sweep
=======
...
<<<<<<< HEAD
FM_RESTORATION_GAP_LOG_MAX_BYTES=262144   # size cap for the arm-owned restoration-gap ledger (state/.restoration-gaps.log)
FM_RESTORATION_GAP_LOG_KEEP_LINES=1000   # newest complete restoration-gap rows considered when the ledger is capped
...
<<<<<<< HEAD
FM_CLASSIFY_DONE_PENDING_VERIFY_VERB=done-pending-verify   # leading status verb for a worker-declared done-awaiting-verification wait; a parked expected idle, excluded from FM_CAPTAIN_RE and absorbed on the FM_PAUSE_RESURFACE_SECS recheck cadence like paused:/captain-held:
...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart; a declared external wait, verified captain-held transfer, or worker-declared done-pending-verify takes the FM_PAUSE_RESURFACE_SECS recheck below instead
FM_PAUSE_RESURFACE_SECS=3600       # seconds between bounded rechecks of a declared external wait, verified captain-held transfer, or worker-declared done-pending-verify, including a live idle pane after its first inconclusive stale wake and a live busy pane past FM_BUSY_TURN_MAX_SECS; the away-mode daemon uses the same setting, ageing its window against the crew's own latest status line rather than pane busy state
FM_DEAD_WORKER_GRACE=120    # minimum age of a task's state/<id>.meta before its missing endpoint may be declared a dead worker; a fresh spawn or relaunch rewrites the meta, so this gives the worker time to start before the reality check would escalate
FM_DEAD_WORKER_LOADAVG=     # test override for the 1-minute load average used by the dead-worker machine-idle verdict; unset means the real machine load from /proc/loadavg or vm.loadavg
FM_DEAD_WORKER_KERNEL_LOG=  # test override for the dead-worker OOM-evidence reader: an executable whose stdout is a kernel log; unset reads dmesg on Linux with passwordless sudo journalctl -k as the fallback
FM_STOP_VERIFY_CONTROL=     # stop-verification control-plane binary (bin/fm-stop-verify.sh); default is fm-control.sh next to it
FM_STOP_VERIFY_STATE_BIN=   # stop-verification agent-state reader: an executable whose stdout is one agent-state word for <task> <window> <backend>; unset reads the real backend classifier
FM_STOP_VERIFY_LOG=         # stop-verification honest event log (state/.stop-verify.log by default)
FM_STOP_VERIFY_COOLDOWN=300  # seconds a victim whose stop is unconfirmed stays in cooldown before a re-addressing may go straight to the harder interrupt
