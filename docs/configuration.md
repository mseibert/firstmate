...
The scan applies the merge policy in `~/.claude/pr-policy.md` (or `FM_PR_GREEN_RETURN_POLICY`): a repo the policy's posture holds (not in the `autonomous` rows of an `allowlist` policy, or on the `ask`/`deny` wait list of a `denylist` policy), a policy hard stop, an unreadable or unrecognized-posture policy, or a changed-file list that cannot be proven complete holds the PR instead of merging it, and the queued wake names that reason for the captain.
