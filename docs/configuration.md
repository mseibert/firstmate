...
An otherwise due GitHub PR is also held, because the GitHub merge path cannot bind the merge to the verified head; Forgejo (`head_commit_id`) and GitLab (`--sha`) carry the bound-merge mandate, and the mandate names the verified head with `--expected-head`, which the merge path refuses to replace with a moved one.
